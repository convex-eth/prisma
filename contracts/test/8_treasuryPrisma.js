...
const TreasuryManagerPrisma = artifacts.require("TreasuryManagerPrisma");
const IPrismaLPStaking = artifacts.require("IPrismaLPStaking");
...
contract("Test swapping/converting/lping and other actions for treasury", async accounts => {
  it("should test treasury actions", async () => {
    ...
    // let manager = await TreasuryManagerPrisma.new({from:deployer});
    let manager = await TreasuryManagerPrisma.at(contractList.system.treasuryManagerPrisma);
    console.log("manager: " +manager.address);

    var approve = prisma.contract.methods.approve(manager.address,"115792089237316195423570985008687907853269984665640564039457584007913129639935").encodeABI();
    var remove = cvxprisma.contract.methods.approve(contractList.system.treasuryManager,"0").encodeABI();
    ...
    let prismastaking = await IPrismaLPStaking.at("0xd91fBa4919b7BF3B757320ea48bA102F543dE341");

    await incentivevoting.registerAccountWeightAndVote(contractList.system.voteProxy, 52, [[17,10000]],{from:multisig,gasPrice:0});
    console.log("voted weight on prisma pool");
    await advanceTime(day*7);
    await prismastaking.fetchRewards().catch(a=>console.log("revert on fetchRewards: " +a));
    ...
    await manager.claim(true,{from:deployer});
    console.log("\nclaimed lp rewards\n");
