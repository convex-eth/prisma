...
contract("prisma deploy and lock testing", async accounts => {
  it("should successfully run", async () => {
    ...
    let vesting = await IPrismaVesting.at(contractList.prisma.vesting);
    let vestclaim = await VestingClaim.new(voteproxy.address, cvxPrisma.address, prismaLocker.address, contractList.prisma.vesting );
    console.log("vestclaim: " +vestclaim.address);

    await booster.setTokenMinter(vestclaim.address, true, {from:deployer});
    ...
    await vesting.setDelegateApproval(vestclaim.address, true, {from:userZ});
    await vestclaim.claimToConvexFull({from:userZ});
    console.log("claimed");
