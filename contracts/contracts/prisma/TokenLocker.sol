...
contract TokenLocker is SystemStart {
    ...
    event LocksWithdrawn(address indexed account, uint withdrawn, uint penalty);
    ...
    constructor(
        ...
        uint _lockToTokenRatio
    ) SystemStart(_addressProvider) {
        lockToken = _token;
    ...
    function getAccountBalances(address account) external view returns (uint256 locked, uint256 unlocked) {
        ...
        uint frozen = accountData.frozen;
     ...
     */
    function getAccountWeightAt(address account, uint week) public view returns (uint256) {
        if (week > getWeek()) return 0;
        uint32[65535] storage weeklyUnlocks = accountWeeklyUnlocks[account];
        uint40[65535] storage weeklyWeights = accountWeeklyWeights[account];
        AccountData storage accountData = accountLockData[account];

        uint accountWeek = accountData.week;
        if (accountWeek >= week) return weeklyWeights[week];

        uint locked = accountData.locked;
        uint weight = weeklyWeights[accountWeek];
        ...
        while (accountWeek < systemWeek) {
            ...
            if (bitfield & uint256(1) == 1) {
                uint amount = weeklyUnlocks[accountWeek];
                locked -= amount;
                if (locked == 0) break;
            }
        }
    ...
    function getWithdrawWithPenaltyAmounts(
    ...
    ) external view returns (uint amountWithdrawn, uint penaltyAmountPaid) {
        AccountData storage accountData = accountLockData[account];
        uint32[65535] storage unlocks = accountWeeklyUnlocks[account];

        // first we apply the unlocked balance without penalty
        uint unlocked = accountData.unlocked;
        if (unlocked >= amountToWithdraw) {
            return (amountToWithdraw, 0);
        }
        uint remaining = amountToWithdraw - unlocked;
        uint penaltyTotal;
        ...
        for (uint weeksToUnlock = 1; weeksToUnlock < MAX_LOCK_WEEKS; weeksToUnlock++) {
            accountWeek++;
     ...
     */
    function getTotalWeightAt(uint week) public view returns (uint256) {
        uint systemWeek = getWeek();
    ...
    function _lock(address _account, uint256 _amount, uint256 _weeks) internal {
        ...
        uint accountWeight = _weeklyWeightWrite(_account);
        uint totalWeight = getTotalWeightWrite();
        uint systemWeek = getWeek();
        uint frozen = accountData.frozen;
        if (frozen > 0) {
            ...
            if (previous == 0) {
                uint idx = unlockWeek / 256;
                uint bitfield = accountData.updateWeeks[idx] | (uint256(1) << (unlockWeek % 256));
    ...
    function extendLock(
        ...
        uint changedWeek = systemWeek + _weeks;
        uint previous = unlocks[changedWeek];
        unlocks[changedWeek] = uint32(previous - _amount);
        totalWeeklyUnlocks[changedWeek] -= uint32(_amount);
        if (previous == _amount) {
            uint idx = changedWeek / 256;
            uint bitfield = accountData.updateWeeks[idx] & ~(uint256(1) << (changedWeek % 256));
            accountData.updateWeeks[idx] = bitfield;
        }
    ...
    function lockMany(address _account, LockData[] calldata newLocks) external notFrozen(_account) returns (bool) {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            uint unlockWeek = systemWeek + week;
            uint previous = unlocks[unlockWeek];
            unlocks[unlockWeek] = uint32(previous + amount);
            totalWeeklyUnlocks[unlockWeek] += uint32(amount);

            if (previous == 0) {
                uint idx = (unlockWeek / 256) - (systemWeek / 256);
    ...
    function extendMany(ExtendLockData[] calldata newExtendLocks) external notFrozen(msg.sender) returns (bool) {
        ...
        uint length = newExtendLocks.length;
        for (uint i = 0; i < length; i++) {
            uint amount = newExtendLocks[i].amount;
            uint oldWeeks = newExtendLocks[i].currentWeeks;
            uint newWeeks = newExtendLocks[i].newWeeks;
            ...
            if (previous == 0) {
                uint idx = (newWeeks / 256) - (systemWeek / 256);
    ...
    function freeze() external notFrozen(msg.sender) {
        ...
        uint256 systemWeek = getWeek();
        uint accountWeight = _weeklyWeightWrite(msg.sender);
    ...
    /**
        ...
        @param amountToWithdraw The amount of tokens to withdraw from active locks. This
                                ...
                                `type(uint).max` withdrawals the entire available locked
     ...
     */
    function withdrawWithPenalty(uint amountToWithdraw) external notFrozen(msg.sender) returns (uint) {
        AccountData storage accountData = accountLockData[msg.sender];
        uint32[65535] storage unlocks = accountWeeklyUnlocks[msg.sender];
        uint256 weight = _weeklyWeightWrite(msg.sender);
        ...
        uint remaining = amountToWithdraw;
        if (unlocked > 0) {
            remaining -= unlocked;
            accountData.unlocked = 0;
        }
        ...
        for (uint weeksToUnlock = 1; weeksToUnlock < MAX_LOCK_WEEKS; weeksToUnlock++) {
            ...
            if ((bitfield >> (systemWeek % 256)) & uint256(1) == 1) {
                ...
                uint penaltyOnAmount = (lockAmount * weeksToUnlock) / MAX_LOCK_WEEKS;
        ...
        if (amountToWithdraw == type(uint).max) {
            amountToWithdraw -= remaining;
        } else {
